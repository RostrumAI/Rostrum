...
3. Treat the user's most recent prompt as the exact Markdown content to write into `$CLAUDE_PROJECT_DIR/hello_world.md`. Do not add headings, wrappers, commentary, or code fences unless the user included them.
4. Use the Bash tool to write that exact content into `$CLAUDE_PROJECT_DIR/hello_world.md`. Do not write a relative path.
5. Use the Bash tool to run `.claude/bin/rostrum-hello-world-state.sh next hello_moon`.
6. Immediately invoke the hidden `rostrum-hello-moon-phase` skill so phase two can request its content.
